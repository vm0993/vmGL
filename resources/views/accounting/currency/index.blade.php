...
@section('content')
    <div class="intro-y flex items-center mt-2">
        <h2 class="intro-y text-lg font-medium mt-auto">Data Currency</h2>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-2">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('currencies.create') }}" class="btn btn-primary shadow-md mr-2">Add New</a>
            <div class="dropdown">
            ...
            <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                <div class="w-56 relative text-gray-700 dark:text-gray-300">
                    <input type="text" class="form-control w-56 box pr-10 placeholder-theme-13" placeholder="Cari">
        ...
        <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        <th class="px-4 py-2 w-10">Code</th>
                        <th class="px-4 py-2">Currency Name</th>
                        <th class="px-4 py-2 w-10">Rate</th>
                        <th class="px-4 py-2 w-10">Symbol</th>
                ...
                <tbody>
                    ...
                    @else
                        @foreach($currencys as $x => $result)
                        <tr class="intro-x">
                            <td class="w-20">
                                <a href="{{ route('currencies.edit',['currency' => $result['id']]) }}">
                                    <span class="font-medium text-theme-20 whitespace-nowrap">{{ $result['code'] }}</span>
                                </a>
                            </td>
                            <td>
                                <span class="font-medium whitespace-nowrap">{{ $result['name'] }}</span>
                            </td>
                            <td>
                                <span class="font-medium whitespace-nowrap text-right">{{ number_format($result['rate']) }}</span>
                            </td>
                            <td>
