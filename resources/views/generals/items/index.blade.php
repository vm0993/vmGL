...
@section('content')
    <div class="intro-y flex items-center mt-2">
        <h2 class="intro-y text-lg font-medium mt-auto">Data Barang/Jasa</h2>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-3">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('items.create') }}" class="btn btn-primary shadow-md mr-2">Add New</a>
            <div class="dropdown">
                ...
                <tbody>
                    ...
                    @else
                        @foreach($results as $x => $result)
                            <tr class="intro-x">
                                <td>
                                    <a href="{{ route('items.edit',['item' => $result['id']]) }}">
                                        <span class="font-medium text-theme-20 whitespace-nowrap">{{ $result['code'] }}</span>
                                    </a>
                                    <div class="font-medium text-theme-26 whitespace-nowrap mt-0.5">{{ $result['name'] }}</div>
                                </td>
                                <td class="w-10">
                                    <span class="font-medium whitespace-nowrap text-center">{{ $result['category'] }}</span>
                                </td>
                                <td class="w-10">
                                    <span class="font-medium whitespace-nowrap text-center">{{ $result['unit'] }}</span>
                                </td>
                                <td>
                                    <span class="text-xs whitespace-nowrap">Buy : {{ $result['purchaseTax'] }}</span>
                                    <div class="text-xs text-theme-26 whitespace-nowrap mt-0.5">Sell : {{ $result['saleTax'] }}</div>
                                </td>
                                <td class="w-10">
                                    <span class="text-xs whitespace-nowrap text-right">Buy : {{ number_format($result['buyPrice']) }}</span>
                                    <div class="text-xs text-theme-26 whitespace-nowrap mt-0.5 text-right">Sell : {{ number_format($result['sellPrice']) }}</div>
                                </td>
                                <td class="w-10">
