...
<div class="grid grid-cols-12 gap-10 mt-5">
    <div class="intro-y col-span-12 lg:col-span-12 md:col-span-12 sm:col-span-12">
        ...
        <form action="{{ route('advance-approvals.update',['advance_approval' => $result['advance']['id']]) }}" method="post" onkeydown="return event.key != 'Enter';">
        @else
        <form action="{{ route('advance-approvals.store') }}" method="post" onkeydown="return event.key != 'Enter';">
        @endif
            @csrf
                    ...
                    <div class="preview">
                        <div class="intro-y">
                                ...
                                <div class="md:grid grid-cols-2 sm:grid grid-cols-2 gap-2">
                                    <div class="md:grid grid-cols-2 sm:grid grid-cols-2 gap-2">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form-control select" disabled name="personel_id" id="personel_id">
                            ...
                            <div class="grid grid-cols-12 gap-x-5 mt-1">
                                <div class="col-span-12 lg:col-span-6 md:col-span-6 sm:col-span-6 mt-1">
                                    <div class="input-group">
                                ...
                                <div class="col-span-12 lg:col-span-3 md:col-span-3 sm:col-span-6 mt-1">
                                    <div class="input-group">
                                        <div class="input-group-text w-full text-center">
                                            <span class="text-theme-20">Approved Amount</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-right mt-5">
                                <a href="{{ route('advance-approvals.index') }}" class="btn closeModal btn-outline-secondary w-20 mr-1"> Cancel</a>
                                <button type="submit" class="btn btn-primary w-auto"> Approved</button>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('footer')
    ...
    <script type="text/javascript">
        var transDate = '';
        var id = 0;
        var url = "{{ route('advance-approvals.index') }}";
        var count = 1;
        ...
        if(addState=='create'){
            ...
            $.get(url + '/'+ id +'/get-transaction',  function(data, status){ 
                for(var a=0;a<data.length;a++){
                    ...
                    html += '    <div class="col-span-12 lg:col-span-6 md:col-span-6 sm:col-span-6 mt-2">';
                    ...
                    html += '            </div>';
                    html += '            <input type="text" step="any" type-currency="IDR" name="amount_'+ a +'" id="amount_'+ a +'" value="'+formatNumber(accounting.toFixed(data[a].amount),2)+'" class="form-control text-right" />';
                    html += '        </div>';
                    html += '    </div>';
                    html += '    <div class="col-span-12 lg:col-span-3 md:col-span-3 sm:col-span-6 mt-2">';
                    html += '        <div class="input-group">';
                    ...
                    html += '                {{ __('advance-request.amount') }}';
                    html += '            </div>';
                    html += '            <input type="text" step="any" type-currency="IDR" name="approve_amount_'+ a +'" id="approve_amount_'+ a +'" class="form-control amount debet text-right" />';
                    html += '        </div>';
                    html += '    </div>';
