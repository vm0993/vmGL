<x-jet-dialog-modal wire:model="create">
    <x-slot name="title">
        {{ ( $this->size_id) ? trans('size.edit-title') : trans('size.add-title') }}
    </x-slot>
    <x-slot name="content">
        ...
        <div class="col-span-12 sm:col-span-3">
            <label for="formCode" class="block text-gray-700 text-sm font-bold mb-2">{{ trans('global.gravity') }}</label>
            <input type="text" step="any" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right" id="formCode" placeholder="Enter Gravity" wire:model="editing.gravity">
            @error('editing.gravity') <span class="text-theme-21 mt-1">{{ $message }}</span>@enderror
        </div>
    </x-slot>
    <x-slot name="footer">
