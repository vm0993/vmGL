...
<div class="grid grid-cols-12 gap-6 mt-5">
    ...
    @if($showEditModal)
        @include('livewire.general.personil-create')
    @endif
        ...
        <table class="table table-report -mt-2">
            <thead>
                <tr class="rounded-md bg-gray-400">
                    ...
                    <th class="whitespace-nowrap">{{ trans('personil.address') }}</th>
                    <th class="text-right whitespace-nowrap">{{ trans('personil.amount') }}</th>
            ...
            <tbody>
                ...
                <tr class="intro-x">
                    <td class="w-20">
                        <a href="javascript:;" wire:click="edit({{ $personil->id }})">
                            <span class="font-medium text-theme-20 whitespace-nowrap">{{ $personil->code }}</span>
                        </a>
                    </td>
                    <td>
                        <span class="font-medium whitespace-nowrap">{{ $personil->name }}</span>
                    </td>
                    <td class="text-left">{{ $personil->address }}</td>
                    <td class="text-right">{{ number_format($personil->amount) }}</td>
                    <td class="table-report__action w-20">
                        <div class="flex justify-center items-center">
                            <button wire:click="confirmingDeletion({{ $personil->id }})" class="flex items-center text-theme-21" data-toggle="modal" data-target="#delete-confirmation-modal">
    ...
    <div class="intro-y flex flex-wrap sm:flex-row sm:flex-nowrap items-right mt-3">
        {{ $personels->links('pagination',['is_livewire' => true]) }}
    </div>
    <!-- END: Pagingation -->
</div>