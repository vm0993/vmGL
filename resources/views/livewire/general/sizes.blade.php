...
<div class="grid grid-cols-12 gap-6 mt-5">
    ...
    @if($showEditModal)
        @include('livewire.general.size-create')
    @endif
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <table class="table table-report -mt-2">
            ...
            <tbody>
                @foreach($gSize as $size)
                <tr class="intro-x">
                    <td class="w-20">
                        <a href="javascript:;" wire:click="edit({{ $size->id }})">
                            <span class="font-medium text-theme-20 whitespace-nowrap">{{ $size->code }}</span>
                        </a>
                        <div class="text-gray-600 text-xs whitespace-nowrap mt-0.5">{{ number_format($size->gravity) }}</div>
                    </td>
                    <td>
                        <span class="font-medium whitespace-nowrap">{{ $size->name }}</span>
                    </td>
                    <td>
                        <span class="font-medium whitespace-nowrap">{{ $size->status == 0 ? "Active" : "Suspend" }}</span>
                    </td>
                    <td class="table-report__action w-10">
                        <div class="flex justify-center items-center">
                            <button wire:click="confirmingDeletion({{ $size->id }})" wire:loading.attr="disabled" class="flex items-center text-theme-21">
    ...
    <div class="intro-y flex flex-wrap sm:flex-row sm:flex-nowrap items-right mt-3">
        {{ $gSize->links('pagination',['is_livewire' => true]) }}
    </div>
    <!-- END: Pagingation -->
</div>